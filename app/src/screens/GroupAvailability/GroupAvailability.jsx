import React from "react";
import "./style.css";

export const GroupAvailability = () => {
  return (
    <div className="group-availability">
      <div className="overlap-wrapper">
        <div className="overlap">
          <div className="container">
            <div className="overlap-group">
              <p className="element">
                0/4 Available
                <br />
                Mouseover the Calendar to See Who Is Available
              </p>

              <div className="rectangle" />

              <div className="div" />

              <div className="rectangle-2" />
            </div>

            <div className="group-s-availability">Group&#39;s Availability</div>

            <div className="overlap-2">
              <div className="text-wrapper">5:00PM</div>

              <div className="text-wrapper-2">6:00PM</div>
            </div>

            <div className="text-wrapper-3">October 2024</div>

            <div className="text-wrapper-4">4/4 Available</div>

            <div className="overlap-3">
              <div className="text-wrapper-5">Wed</div>

              <div className="text-wrapper-6">3</div>
            </div>

            <div className="text-wrapper-7">Time/Day</div>

            <div className="overlap-4">
              <div className="text-wrapper-8">Sun</div>

              <div className="text-wrapper-9">7</div>
            </div>

            <div className="text-wrapper-10">10:00 AM</div>

            <div className="text-wrapper-11">11:00 AM</div>

            <div className="text-wrapper-12">12:00 PM</div>

            <div className="text-wrapper-13">10:00 PM</div>

            <div className="text-wrapper-14">11:00 PM</div>

            <div className="overlap-5">
              <div className="text-wrapper-15">Sat</div>

              <div className="text-wrapper-16">6</div>

              <div className="rectangle-3" />
            </div>

            <div className="text-wrapper-17">4:00 PM</div>

            <div className="text-wrapper-18">6:00 AM</div>

            <div className="text-wrapper-19">7:00 AM</div>

            <div className="text-wrapper-20">8:00 AM</div>

            <div className="text-wrapper-21">9:00 AM</div>

            <div className="text-wrapper-22">1:00 PM</div>

            <div className="text-wrapper-23">2:00 PM</div>

            <div className="text-wrapper-24">3:00 PM</div>

            <div className="text-wrapper-25">7:00 PM</div>

            <div className="text-wrapper-26">8:00PM</div>

            <div className="text-wrapper-27">9:00 PM</div>

            <div className="overlap-6">
              <div className="text-wrapper-28">Fri</div>

              <div className="text-wrapper-29">5</div>
            </div>

            <img
              className="caret-right"
              alt="Caret right"
              src="https://c.animaapp.com/tBO78KcB/img/caret-right-1-1.svg"
            />

            <img
              className="caret-left"
              alt="Caret left"
              src="https://c.animaapp.com/tBO78KcB/img/caret-left-2-1.svg"
            />

            <div className="text-wrapper-30">Mon</div>

            <div className="text-wrapper-31">Thu</div>

            <div className="text-wrapper-32">Tue</div>

            <div className="text-wrapper-33">K</div>

            <div className="text-wrapper-34">2</div>

            <div className="text-wrapper-35">4</div>

            <div className="text-wrapper-36">1</div>

            <div className="rectangle-4" />

            <div className="rectangle-5" />

            <div className="rectangle-6" />

            <div className="rectangle-7" />

            <div className="rectangle-8" />

            <div className="rectangle-9" />

            <div className="rectangle-10" />

            <div className="rectangle-11" />

            <div className="rectangle-12" />

            <div className="rectangle-13" />

            <div className="rectangle-14" />

            <div className="rectangle-15" />

            <div className="rectangle-16" />

            <div className="rectangle-17" />

            <div className="rectangle-18" />

            <div className="overlap-7">
              <div className="rectangle-19" />

              <div className="rectangle-20" />
            </div>

            <div className="rectangle-21" />

            <div className="rectangle-22" />

            <div className="rectangle-23" />

            <div className="rectangle-24" />

            <div className="rectangle-25" />

            <div className="rectangle-26" />

            <div className="rectangle-27" />

            <div className="rectangle-28" />

            <div className="rectangle-29" />

            <div className="rectangle-30" />

            <div className="overlap-8">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-33" />

            <div className="rectangle-34" />

            <div className="rectangle-35" />

            <div className="rectangle-36" />

            <div className="rectangle-37" />

            <div className="rectangle-38" />

            <div className="overlap-9">
              <div className="rectangle-39" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-40" />

            <div className="rectangle-41" />

            <div className="rectangle-42" />

            <div className="rectangle-43" />

            <div className="rectangle-44" />

            <div className="rectangle-45" />

            <div className="overlap-10">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-46" />

            <div className="rectangle-47" />

            <div className="rectangle-48" />

            <div className="rectangle-49" />

            <div className="rectangle-50" />

            <div className="rectangle-51" />

            <div className="overlap-11">
              <div className="rectangle-39" />

              <div className="rectangle-52" />
            </div>

            <div className="rectangle-53" />

            <div className="rectangle-54" />

            <div className="rectangle-55" />

            <div className="rectangle-56" />

            <div className="overlap-12">
              <div className="rectangle-57" />

              <div className="rectangle-58" />
            </div>

            <div className="rectangle-59" />

            <div className="rectangle-60" />

            <div className="rectangle-61" />

            <div className="rectangle-62" />

            <div className="overlap-13">
              <div className="rectangle-57" />

              <div className="rectangle-58" />
            </div>

            <div className="rectangle-63" />

            <div className="rectangle-64" />

            <div className="rectangle-65" />

            <div className="rectangle-66" />

            <div className="rectangle-67" />

            <div className="rectangle-68" />

            <div className="rectangle-69" />

            <div className="rectangle-70" />

            <div className="rectangle-71" />

            <div className="rectangle-72" />

            <div className="rectangle-73" />

            <div className="rectangle-74" />

            <div className="rectangle-75" />

            <div className="rectangle-76" />

            <div className="rectangle-77" />

            <div className="rectangle-78" />

            <div className="rectangle-79" />

            <div className="rectangle-80" />

            <div className="rectangle-81" />

            <div className="rectangle-82" />

            <div className="rectangle-83" />

            <div className="rectangle-84" />

            <div className="rectangle-85" />

            <div className="rectangle-86" />

            <div className="rectangle-87" />

            <div className="rectangle-88" />

            <div className="rectangle-89" />

            <div className="rectangle-90" />

            <div className="rectangle-91" />

            <div className="rectangle-92" />

            <div className="rectangle-93" />

            <div className="rectangle-94" />

            <div className="rectangle-95" />

            <div className="rectangle-96" />

            <div className="rectangle-97" />

            <div className="rectangle-98" />

            <div className="rectangle-99" />

            <div className="rectangle-100" />

            <div className="rectangle-101" />

            <div className="rectangle-102" />

            <div className="rectangle-103" />

            <div className="rectangle-104" />

            <div className="overlap-group-2">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-105" />

            <div className="rectangle-106" />

            <div className="rectangle-107" />

            <div className="rectangle-108" />

            <div className="rectangle-109" />

            <div className="overlap-14">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-110" />

            <div className="rectangle-111" />

            <div className="rectangle-112" />

            <div className="rectangle-113" />

            <div className="overlap-15">
              <div className="rectangle-31" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-115" />

            <div className="rectangle-116" />

            <div className="rectangle-117" />

            <div className="rectangle-118" />

            <div className="overlap-16">
              <div className="rectangle-119" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-120" />

            <div className="rectangle-121" />

            <div className="rectangle-122" />

            <div className="rectangle-123" />

            <div className="rectangle-124" />

            <div className="rectangle-125" />

            <div className="rectangle-126" />

            <div className="rectangle-127" />

            <div className="rectangle-128" />

            <div className="rectangle-129" />

            <div className="rectangle-130" />

            <div className="rectangle-131" />

            <div className="rectangle-132" />
          </div>

          <div className="container-2">
            <div className="overlap-17">
              <p className="availableclick-and">
                Available
                <br />
                Click and Drag to Toggle; Saved Immediately
              </p>

              <div className="text-wrapper-37">Unavailable</div>

              <div className="rectangle-133" />

              <div className="rectangle-134" />
            </div>

            <div className="text-wrapper-38">Your Availability</div>

            <div className="text-wrapper-39">5:00PM</div>

            <div className="text-wrapper-40">October 2024</div>

            <div className="overlap-18">
              <div className="text-wrapper-5">Wed</div>

              <div className="text-wrapper-41">3</div>
            </div>

            <div className="text-wrapper-42">10:00 AM</div>

            <div className="text-wrapper-43">11:00 AM</div>

            <div className="text-wrapper-44">12:00 PM</div>

            <div className="text-wrapper-45">10:00 PM</div>

            <div className="text-wrapper-46">11:00 PM</div>

            <div className="text-wrapper-47">Time/Day</div>

            <div className="text-wrapper-48">4:00 PM</div>

            <div className="text-wrapper-49">6:00 AM</div>

            <div className="text-wrapper-50">7:00 AM</div>

            <div className="text-wrapper-51">8:00 AM</div>

            <div className="text-wrapper-52">9:00 AM</div>

            <div className="text-wrapper-53">1:00 PM</div>

            <div className="text-wrapper-54">2:00 PM</div>

            <div className="text-wrapper-55">3:00PM</div>

            <div className="text-wrapper-56">7:00 PM</div>

            <div className="text-wrapper-57">8:00 PM</div>

            <div className="text-wrapper-58">9:00 PM</div>

            <div className="overlap-19">
              <div className="text-wrapper-28">Fri</div>

              <div className="text-wrapper-59">5</div>
            </div>

            <img
              className="img"
              alt="Caret left"
              src="https://c.animaapp.com/tBO78KcB/img/caret-left-2-1.svg"
            />

            <div className="text-wrapper-60">Mon</div>

            <div className="text-wrapper-61">Thu</div>

            <div className="text-wrapper-62">Sun</div>

            <div className="text-wrapper-63">Tue</div>

            <div className="text-wrapper-64">Sat</div>

            <div className="text-wrapper-65">2</div>

            <div className="text-wrapper-66">4</div>

            <div className="text-wrapper-67">1</div>

            <div className="text-wrapper-68">6</div>

            <div className="text-wrapper-69">7</div>

            <div className="text-wrapper-70">6:00PM</div>

            <div className="rectangle-135" />

            <div className="rectangle-136" />

            <div className="rectangle-137" />

            <div className="rectangle-138" />

            <div className="rectangle-139" />

            <div className="rectangle-140" />

            <div className="rectangle-141" />

            <div className="rectangle-142" />

            <div className="rectangle-143" />

            <div className="rectangle-144" />

            <div className="rectangle-145" />

            <div className="rectangle-146" />

            <div className="rectangle-147" />

            <div className="rectangle-148" />

            <div className="rectangle-149" />

            <div className="overlap-20">
              <div className="rectangle-19" />

              <div className="rectangle-20" />
            </div>

            <div className="rectangle-150" />

            <div className="rectangle-151" />

            <div className="rectangle-152" />

            <div className="rectangle-153" />

            <div className="rectangle-154" />

            <div className="rectangle-155" />

            <div className="rectangle-156" />

            <div className="rectangle-157" />

            <div className="rectangle-158" />

            <div className="rectangle-159" />

            <div className="overlap-21">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-160" />

            <div className="rectangle-161" />

            <div className="rectangle-162" />

            <div className="rectangle-163" />

            <div className="rectangle-164" />

            <div className="rectangle-165" />

            <div className="overlap-22">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-166" />

            <div className="rectangle-167" />

            <div className="rectangle-168" />

            <div className="rectangle-169" />

            <div className="rectangle-170" />

            <div className="rectangle-171" />

            <div className="overlap-23">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-172" />

            <div className="rectangle-173" />

            <div className="rectangle-174" />

            <div className="rectangle-175" />

            <div className="rectangle-176" />

            <div className="rectangle-177" />

            <div className="overlap-24">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-178" />

            <div className="rectangle-179" />

            <div className="rectangle-180" />

            <div className="rectangle-181" />

            <div className="rectangle-182" />

            <div className="rectangle-183" />

            <div className="overlap-25">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-184" />

            <div className="rectangle-185" />

            <div className="rectangle-186" />

            <div className="rectangle-187" />

            <div className="rectangle-188" />

            <div className="rectangle-189" />

            <div className="overlap-26">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-190" />

            <div className="rectangle-191" />

            <div className="rectangle-192" />

            <div className="rectangle-193" />

            <div className="overlap-27">
              <div className="rectangle-31" />

              <div className="rectangle-194" />
            </div>

            <div className="rectangle-195" />

            <div className="rectangle-196" />

            <div className="rectangle-197" />

            <div className="rectangle-198" />

            <div className="overlap-28">
              <div className="rectangle-31" />

              <div className="rectangle-194" />
            </div>

            <div className="rectangle-199" />

            <div className="rectangle-200" />

            <div className="rectangle-201" />

            <div className="rectangle-202" />

            <div className="rectangle-203" />

            <div className="rectangle-204" />

            <div className="rectangle-205" />

            <div className="rectangle-206" />

            <div className="rectangle-207" />

            <div className="rectangle-208" />

            <div className="rectangle-209" />

            <div className="rectangle-210" />

            <div className="rectangle-211" />

            <div className="rectangle-212" />

            <div className="rectangle-213" />

            <div className="rectangle-214" />

            <div className="rectangle-215" />

            <div className="rectangle-216" />

            <div className="rectangle-217" />

            <div className="rectangle-218" />

            <div className="rectangle-219" />

            <div className="rectangle-220" />

            <div className="rectangle-221" />

            <div className="rectangle-222" />

            <div className="rectangle-223" />

            <div className="rectangle-224" />

            <div className="rectangle-225" />

            <div className="rectangle-226" />

            <div className="rectangle-227" />

            <div className="rectangle-228" />

            <div className="rectangle-229" />

            <div className="rectangle-230" />

            <div className="overlap-29">
              <div className="rectangle-31" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-231" />

            <div className="rectangle-232" />

            <div className="rectangle-233" />

            <div className="rectangle-234" />

            <div className="overlap-30">
              <div className="rectangle-119" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-235" />

            <div className="rectangle-236" />

            <div className="rectangle-237" />

            <div className="rectangle-238" />

            <div className="rectangle-239" />

            <div className="rectangle-240" />

            <div className="rectangle-241" />

            <div className="rectangle-242" />

            <div className="rectangle-243" />

            <div className="rectangle-244" />

            <div className="rectangle-245" />

            <div className="rectangle-246" />

            <div className="rectangle-247" />

            <div className="rectangle-248" />

            <div className="rectangle-249" />

            <div className="rectangle-250" />

            <div className="rectangle-251" />

            <div className="rectangle-252" />

            <div className="rectangle-253" />

            <div className="rectangle-254" />
          </div>

          <div className="container-3">
            <div className="header-menu">
              <div className="frame">
                <div className="text-wrapper-71">Schedule</div>
              </div>

              <div className="div-wrapper">
                <div className="text-wrapper-72">Availability</div>
              </div>

              <div className="frame-2">
                <div className="text-wrapper-73">Community</div>
              </div>

              <div className="frame-3">
                <div className="text-wrapper-73">Map</div>
              </div>
            </div>

            <div className="text-wrapper-74">Pickup@Penn</div>

            <img
              className="image"
              alt="Image"
              src="https://c.animaapp.com/tBO78KcB/img/image-1@2x.png"
            />

            <img
              className="image-2"
              alt="Image"
              src="https://c.animaapp.com/tBO78KcB/img/image-29@2x.png"
            />
          </div>

          <div className="container-4">
            <div className="overlap-group">
              <p className="element">
                0/4 Available
                <br />
                Mouseover the Calendar to See Who Is Available
              </p>

              <div className="rectangle" />

              <div className="div" />

              <div className="rectangle-2" />
            </div>

            <div className="group-s-availability">Group&#39;s Availability</div>

            <div className="overlap-2">
              <div className="text-wrapper">5:00PM</div>

              <div className="text-wrapper-2">6:00PM</div>
            </div>

            <div className="text-wrapper-3">October 2024</div>

            <div className="text-wrapper-4">4/4 Available</div>

            <div className="overlap-3">
              <div className="text-wrapper-5">Wed</div>

              <div className="text-wrapper-6">3</div>
            </div>

            <div className="text-wrapper-7">Time/Day</div>

            <div className="overlap-4">
              <div className="text-wrapper-8">Sun</div>

              <div className="text-wrapper-9">7</div>
            </div>

            <div className="text-wrapper-10">10:00 AM</div>

            <div className="text-wrapper-11">11:00 AM</div>

            <div className="text-wrapper-12">12:00 PM</div>

            <div className="text-wrapper-13">10:00 PM</div>

            <div className="text-wrapper-14">11:00 PM</div>

            <div className="overlap-5">
              <div className="text-wrapper-15">Sat</div>

              <div className="text-wrapper-16">6</div>

              <div className="rectangle-3" />
            </div>

            <div className="text-wrapper-17">4:00 PM</div>

            <div className="text-wrapper-18">6:00 AM</div>

            <div className="text-wrapper-19">7:00 AM</div>

            <div className="text-wrapper-20">8:00 AM</div>

            <div className="text-wrapper-21">9:00 AM</div>

            <div className="text-wrapper-22">1:00 PM</div>

            <div className="text-wrapper-23">2:00 PM</div>

            <div className="text-wrapper-24">3:00 PM</div>

            <div className="text-wrapper-25">7:00 PM</div>

            <div className="text-wrapper-26">8:00PM</div>

            <div className="text-wrapper-27">9:00 PM</div>

            <div className="overlap-6">
              <div className="text-wrapper-28">Fri</div>

              <div className="text-wrapper-29">5</div>
            </div>

            <img
              className="caret-right"
              alt="Caret right"
              src="https://c.animaapp.com/tBO78KcB/img/caret-right-1-1.svg"
            />

            <img
              className="caret-left"
              alt="Caret left"
              src="https://c.animaapp.com/tBO78KcB/img/caret-left-2-1.svg"
            />

            <div className="text-wrapper-30">Mon</div>

            <div className="text-wrapper-31">Thu</div>

            <div className="text-wrapper-32">Tue</div>

            <div className="text-wrapper-34">2</div>

            <div className="text-wrapper-35">4</div>

            <div className="text-wrapper-36">1</div>

            <div className="rectangle-4" />

            <div className="rectangle-5" />

            <div className="rectangle-6" />

            <div className="rectangle-7" />

            <div className="rectangle-8" />

            <div className="rectangle-9" />

            <div className="rectangle-10" />

            <div className="rectangle-11" />

            <div className="rectangle-12" />

            <div className="rectangle-13" />

            <div className="rectangle-14" />

            <div className="rectangle-15" />

            <div className="rectangle-16" />

            <div className="rectangle-17" />

            <div className="rectangle-18" />

            <div className="overlap-7">
              <div className="rectangle-19" />

              <div className="rectangle-20" />
            </div>

            <div className="rectangle-21" />

            <div className="rectangle-22" />

            <div className="rectangle-23" />

            <div className="rectangle-24" />

            <div className="rectangle-25" />

            <div className="rectangle-26" />

            <div className="rectangle-27" />

            <div className="rectangle-28" />

            <div className="rectangle-29" />

            <div className="rectangle-30" />

            <div className="overlap-8">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-33" />

            <div className="rectangle-34" />

            <div className="rectangle-35" />

            <div className="rectangle-36" />

            <div className="rectangle-37" />

            <div className="rectangle-38" />

            <div className="overlap-9">
              <div className="rectangle-39" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-40" />

            <div className="rectangle-41" />

            <div className="rectangle-42" />

            <div className="rectangle-43" />

            <div className="rectangle-44" />

            <div className="rectangle-45" />

            <div className="overlap-10">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-46" />

            <div className="rectangle-47" />

            <div className="rectangle-48" />

            <div className="rectangle-49" />

            <div className="rectangle-50" />

            <div className="rectangle-51" />

            <div className="overlap-11">
              <div className="rectangle-39" />

              <div className="rectangle-52" />
            </div>

            <div className="rectangle-53" />

            <div className="rectangle-54" />

            <div className="rectangle-55" />

            <div className="rectangle-56" />

            <div className="overlap-12">
              <div className="rectangle-57" />

              <div className="rectangle-58" />
            </div>

            <div className="rectangle-59" />

            <div className="rectangle-60" />

            <div className="rectangle-61" />

            <div className="rectangle-62" />

            <div className="overlap-13">
              <div className="rectangle-57" />

              <div className="rectangle-58" />
            </div>

            <div className="rectangle-63" />

            <div className="rectangle-64" />

            <div className="rectangle-65" />

            <div className="rectangle-66" />

            <div className="rectangle-67" />

            <div className="rectangle-68" />

            <div className="rectangle-69" />

            <div className="rectangle-70" />

            <div className="rectangle-71" />

            <div className="rectangle-72" />

            <div className="rectangle-73" />

            <div className="rectangle-74" />

            <div className="rectangle-75" />

            <div className="rectangle-76" />

            <div className="rectangle-77" />

            <div className="rectangle-78" />

            <div className="rectangle-79" />

            <div className="rectangle-80" />

            <div className="rectangle-81" />

            <div className="rectangle-82" />

            <div className="rectangle-83" />

            <div className="rectangle-84" />

            <div className="rectangle-85" />

            <div className="rectangle-86" />

            <div className="rectangle-87" />

            <div className="rectangle-88" />

            <div className="rectangle-89" />

            <div className="rectangle-90" />

            <div className="rectangle-91" />

            <div className="rectangle-92" />

            <div className="rectangle-93" />

            <div className="rectangle-94" />

            <div className="rectangle-95" />

            <div className="rectangle-96" />

            <div className="rectangle-97" />

            <div className="rectangle-98" />

            <div className="rectangle-99" />

            <div className="rectangle-100" />

            <div className="rectangle-101" />

            <div className="rectangle-102" />

            <div className="rectangle-103" />

            <div className="rectangle-104" />

            <div className="overlap-group-2">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-105" />

            <div className="rectangle-106" />

            <div className="rectangle-107" />

            <div className="rectangle-108" />

            <div className="rectangle-109" />

            <div className="overlap-14">
              <div className="rectangle-31" />

              <div className="rectangle-32" />
            </div>

            <div className="rectangle-110" />

            <div className="rectangle-111" />

            <div className="rectangle-112" />

            <div className="rectangle-113" />

            <div className="overlap-15">
              <div className="rectangle-31" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-115" />

            <div className="rectangle-116" />

            <div className="rectangle-117" />

            <div className="rectangle-118" />

            <div className="overlap-16">
              <div className="rectangle-119" />

              <div className="rectangle-114" />
            </div>

            <div className="rectangle-120" />

            <div className="rectangle-121" />

            <div className="rectangle-122" />

            <div className="rectangle-123" />

            <div className="rectangle-124" />

            <div className="rectangle-125" />

            <div className="rectangle-126" />

            <div className="rectangle-127" />

            <div className="rectangle-128" />

            <div className="rectangle-129" />

            <div className="rectangle-130" />

            <div className="rectangle-131" />

            <div className="rectangle-132" />
          </div>
        </div>
      </div>
    </div>
  );
};
